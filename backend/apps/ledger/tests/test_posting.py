...
def test_residential_rent_credits_4110(residential_tenant):
    ...
    entry = post_payment(payment)
    credit_codes = [ln.account.code for ln in entry.lines.all() if ln.credit > 0]
...
def test_late_fee_payment_balances(residential_tenant):
    ...
    credit_codes = [ln.account.code for ln in lines if ln.credit > 0]
...
def test_deposit_payment_balances(residential_tenant):
    ...
    lines = list(entry.lines.all())
    assert sum(ln.debit for ln in lines) == sum(ln.credit for ln in lines)
    debit_codes = [ln.account.code for ln in lines if ln.debit > 0]
    credit_codes = [ln.account.code for ln in lines if ln.credit > 0]
